...
class WDS_CMB2_Attached_Posts_Field {
	...
	public function list_item( $object, $li_class, $icon_class = 'dashicons-plus' ) {
		// Build our list item
		printf(
			'<li data-id="%1$d" class="%2$s" target="_blank">%3$s<a title="' . __( 'Edit' ) . '" href="%4$s">%1$d - %5$s</a>%6$s<span class="dashicons %7$s add-remove"></span></li>',
			$this->get_id( $object ),
			$li_class,
	...
	protected static function setup_scripts() {
		...
		****/
		...
		wp_enqueue_script( 'cmb2-attached-posts-script', get_stylesheet_directory_uri().'/libraries/metabox/extensions/attached-posts/attached-posts.js', $requirements, self::VERSION, true );
		wp_enqueue_style( 'cmb2-attached-posts-style', get_stylesheet_directory_uri().'/libraries/metabox/extensions/attached-posts/attached-posts-admin.css', array(), self::VERSION );
	...
	public function _display_render( $field, $val ) {
		...
		if ( ! empty( $posts ) ) {
			...
			foreach ( (array) $posts as $id => $post ) {
				...
				//$return .= '<li id="attached-'.$id.'"><a href="'.$post['edit_link'].'">'.$post['title'].' - '.$id.'</a></li>';
			}
			$return .= '</ol>';
		} else {
			$return .= '&mdash;';
