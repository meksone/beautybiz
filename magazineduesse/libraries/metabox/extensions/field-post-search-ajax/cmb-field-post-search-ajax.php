...
if( ! class_exists( 'MAG_CMB2_Field_Post_Search_Ajax' ) ) {
	...
	class MAG_CMB2_Field_Post_Search_Ajax {
		...
		public function render( $field, $value, $object_id, $object_type, $field_type ) {	
			...
			if($field->args( 'limit' ) > 1){
				...
				if( isset($value) && !empty($value) ){
					...
					foreach($value as $val){
						...
						*/
						echo '<li>'.$handle.'<input type="hidden" name="'.$field_name.'_results[]" value="'.$val.'"><a href="'.$guid.'" target="_blank" class="edit-link">#'.$val.' - '.$title.'</a><a class="remover"><span class="dashicons dashicons-no"></span><span class="dashicons dashicons-dismiss"></span></a></li>';
					}
				}
				echo '</ul>';			
				$field_value = '';
				...
				if( $field->args( 'object_type' ) == 'user' ){
					...
					*/
					$field_value = ($value ? '#'.$value.' - '.get_the_title($value) : '');
				}
			...
			/* custom */
			if($field->args( 'limit' ) == 1){
				echo ' -> <a href="'.get_edit_post_link($value).'" target="_blank" class="edit-link">'.$field_value.'</a>';
			}
			/* end. custom */
		...
		public function setup_admin_scripts() {
			...
			*/
			wp_register_script( 'jquery-autocomplete', get_stylesheet_directory_uri().'/libraries/metabox/extensions/field-post-search-ajax/js/jquery.autocomplete.min.js', array( 'jquery' ), self::VERSION );
			wp_register_script( 'mag-post-search-ajax', get_stylesheet_directory_uri().'/libraries/metabox/extensions/field-post-search-ajax/js/mag-post-search-ajax.js', array( 'jquery', 'jquery-autocomplete', 'jquery-ui-sortable' ), self::VERSION );
			...
			*/
			wp_enqueue_style( 'mag-post-search-ajax', get_stylesheet_directory_uri().'/libraries/metabox/extensions/field-post-search-ajax/css/mag-post-search-ajax.css', array(), self::VERSION );
