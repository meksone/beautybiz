...
if (! class_exists('description_walker')) {
		class description_walker extends Walker_Nav_Menu {
			function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
				///global $wp_query;
				global $wp_query, $wpdb, $pincuter_theme_opt;
				...
				*/
				...
				$atts['title'] = '';
				if( $nactive ){
					$atts['title'] = ! empty( $item->attr_title ) ? $item->attr_title : 'Sei in '.$item->title;
				} else {
					$atts['title'] = ! empty( $item->attr_title ) ? $item->attr_title : 'Vai in '.$item->title;
				}
				...
				/*
				$subindicator = $pincuter_theme_opt['pnctredux_subindicator_opt'];
				if ( $subindicator == 'yes' ) {
					if ( $has_children > 0 ) {
