...
function _pincuter_id_column_content( $column, $id ) {
...
// for posts
add_action( 'manage_posts_custom_column', '_pincuter_manage_columns', 10, 2 );
add_filter( 'manage_posts_columns', '_pincuter_edit_columns' );
// for pages
add_action('manage_pages_custom_column', '_pincuter_manage_columns', 10, 2);
add_filter('manage_edit-page_columns', '_pincuter_edit_columns');

function _pincuter_manage_columns( $column, $post_id ) {
...
function _pincuter_edit_columns( $columns ) {
...
// for posts
add_filter( 'manage_posts_columns', '_pincuter_AddThumbColumn' );
add_action( 'manage_posts_custom_column', '_pincuter_AddThumbValue', 10, 2 );
// for pages
add_filter( 'manage_pages_columns', '_pincuter_AddThumbColumn' );
add_action( 'manage_pages_custom_column', '_pincuter_AddThumbValue', 10, 2 );

function _pincuter_AddThumbColumn($cols) {
	$cols['thumbnail'] = __("Thumbnail", CURRENT_DOMAIN);
	return $cols;
}
function _pincuter_AddThumbValue($column_name, $post_id) {
	$width = (int) 35;
	$height = (int) 35;
...
/*
// Add Thumb Column
if ( !function_exists('_pincuter_AddThumbColumn') && function_exists('add_theme_support') ) {
	// for post and page
