...
<section id="frontpage_primissimo_piano" class="frontpage_primissimo_piano frontpage_loop">
    <div class="row">
        <?php 
        $primissimo_args = array(
			'post_type' => 'post',
			'post_status' => 'publish',
		...
		$primissimo_query = new WP_Query($primissimo_args);
		if ($primissimo_query->have_posts()) :	
			while ($primissimo_query->have_posts()) : $primissimo_query->the_post();
				$primissimo_id = $primissimo_query->post->ID;
				$do_not_duplicate[] = $primissimo_id;

                $primissimo_title = get_the_title($primissimo_id);
				$primissimo_content = $primissimo_query->post->post_content;

                $home_post_id = $primissimo_id;
				$home_post_title = $primissimo_title;
				$home_post_content = $primissimo_content;
                ?>
                <div class="col-md-12 col_item post_item">
                    <article <?php post_class(); ?>>
                        <div class="post_thumb" data-file-thumb-src="_home_thumb_primissimo">
                            <?php 
							//include(get_stylesheet_directory() . '/templates/home/home_thumb/_home_thumb_primissimo.php');
                            ...
                            echo '<figure class="thumb_normal" data-image-id="'.$home_thumb_image_id.'">';
                                echo '<a href="'.get_permalink($home_post_id).'" title="'.$home_post_title.'">';
                                    echo wp_get_attachment_image( $home_thumb_image_id, 'full', false, array( 'class' => 'img-fluid', 'fetchpriority' => 'high' ) );
                                echo '</a>';
                            echo '</figure>';
							?>
                        </div>
                        <div class="post_desc">
							...
							<?php 
							/*
							$magazineduesse_riassunto_post_breve_contenuto = get_post_meta($primissimo_id, 'magazineduesse_riassunto_post_breve_contenuto', true);
    ...
    <div class="row">
		...
		if ($primo_query->have_posts()) :	
			while ($primo_query->have_posts()) : $primo_query->the_post();
                ...
                ?>
                <div class="col-md-4 col_item post_item">
                    <article <?php post_class(); ?>>
